...
test('record rp animation', async () => {
  ...
  const htmlPath = path.resolve('../landing/public/rp_anim.html')
  ...
  if (!latestVideo) {
    throw new Error('No se genero ningun video webm de RP')
  }

  const srcPath = path.join('videos/', latestVideo.name)
  const destPath = path.resolve('../landing/public/assets/videos/rp-demo.webm')
